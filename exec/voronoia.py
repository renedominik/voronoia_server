...
VOLUME_CMD = os.path.join( TMPL_DIR, "get_volume_newest.exe" )
#VOLUME_CMD = os.path.join( TMPL_DIR, "get_volume_32.exe" )
#VOLUME_CMD = os.path.join( TMPL_DIR, "get_volume_test.exe" )
...
print( 'cwd:', os.getcwd())
print( 'voronoia location:', os.path.realpath( __file__ ))
...
# get_volume.exe ex:0.1 rad:protor i:file.pdb o:out.vol  # x:yes
class Voronoia( CmdTool, ProviMixin, ParallelMixin, RecordsMixin ):
    ...
    args = [
        ...
        _( "keep_waters", type="bool", default=False ),
    ]
    out = [
    ...
    def _post_exec( self ):
        ...
        if not self.parallel:
            ...
            print( "zscore_all", zscore_all, "len(zscore_allb)", len(zscore_allb) )
            if len(zscore_allb) > 0:
